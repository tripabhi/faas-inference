...
run_benchmark() {
    #local concurrency_levels=(5 10 15 20 30 40 50 60 70 80 90 100 200 500 1000)
    local concurrency_levels=(100)
    ...
    for concurrency in "${concurrency_levels[@]}"; do
        ...
        for ((run=1; run<=num_runs; run++)); do
            ...
            ab -n 1000 -c $concurrency -e $result_file -p $payload_file -T "application/json" "$url/$endpoint"
            echo "===================="
        done
    done
}
